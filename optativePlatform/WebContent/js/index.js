...
$(document).ready(function() {
	...
	$("#select1 dd").click(function() {
		$(this).addClass("selected").siblings().removeClass("selected"); //取消同级元素
		if($(this).hasClass("select-all")) { //判断是否是“全部”条件
			...
			if($(".selectA").length > 0) { //检查是否存在同级条件
				$(".selectA a").html($(this).find("a").text());
			} else {
				$(".select-result dl").append(copyThisA.attr("class", "selected selectA"));
			}
			//获取参数，后台查询
			getData();
		}
